...
#pragma omp parallel for
  ...
  T *alloc_vertex_array() {
    char *array =
        (char *) mmap(NULL, sizeof(T) * vertices, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    assert(array != NULL);
    for (int s_i = 0; s_i < sockets; s_i++) {
      ...
      if (dense_selective != nullptr && partitions > 1) {
        ...
        std::thread send_thread([&]() {
          for (int step = 1; step < partitions; step++) {
            ...
            MPI_Send(dense_selective->data + WORD_OFFSET(partition_offset[partition_id]),
                     owned_vertices / 64,
                     MPI_UNSIGNED_LONG,
                     recipient_id,
                     PassMessage,
